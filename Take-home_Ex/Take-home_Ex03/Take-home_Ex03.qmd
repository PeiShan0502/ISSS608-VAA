...
```{r}
mc3_edges$weight <- ifelse(mc3_edges$event2 == "FamilyRelationship", 2, 
                           ifelse(mc3_edges$event2 == "WorksFor", 1,
                                  ifelse(mc3_edges$event3 == "BeneficialOwnership", 4,
                           ifelse(mc3_edges$event3 == "Shareholdership", 3, "Not Applicable"))))
```

Change data type of the weight factor, from character to integer:
...
Check for any missing values in integer columns:

```{r}
print(sum(is.na(mc3_edges$weight)))
print(sum(is.na(mc3_edges$key)))
```

[Step 2:]{.underline}

-   put both mc3_edges and mc3_nodes together to construct network graph
...
-   Build graph visualisation (filter the mc3_edges to show only Beneficial Ownership and Shareholdership, filter the mc3_nodes to show only organisations (i.e., event2 = "Organisation")).
